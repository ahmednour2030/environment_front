...
export default {
  ...
  computed: {
    ...
    getFreelancers() { return this.$store.state.quality.users },
  },
  created() {
    this.$store.dispatch('quality/fetch')
  },
  methods: {
    confirmAccepted(id) {
      this.$swal({
        ...
        if (result.value) {
          ...
          this.$store.dispatch('quality/active', Obj).then(() => {
          })
        }
      })
    },

...
export default {
  ...
  computed: {
    ...
    getCompanies() {
      return this.$store.state.company.usersActive
    },
  },
  created() {
    this.$store.dispatch('company/fetchActive')
  },
  methods: {
    confirmRefuse(id) {
      this.$swal({
        title: 'عملية الغاء التفعيل',
        text: 'تم الغاء تفعيل هذا الشركة بنجاح',
        icon: 'question',
      ...
      this.$swal({
        ...
        if (result.value) {
          this.$swal({
            icon: 'success',
            title: 'عملية حذف',
            text: 'تم تفعيل هذا الشركة بنجاح!',
            customClass: {
              confirmButton: 'btn btn-success',
            },
          })

        ...
        <b-col cols="12">
          ...
          >
            ...
            <template #cell(name)="row">
              {{ row.item.user.name ? row.item.user.name : '' }}
            </template>
...
export default {
  ...
  created() {
    this.$store.dispatch('company/fetchWedding').then(res => {
  ...
  methods: {
    confirmAccepted(id) {
      this.$swal({
        title: 'قبول القاعة',
        text: 'هل تريد موافق على اضافة هذا القاعة',
        icon: 'question',
        ...
        if (result.value) {
          this.$swal({
            icon: 'success',
            title: 'عملية القبول',
            text: 'تم الموافقة على هذا القاعة بنجاح',
            customClass: {
              confirmButton: 'btn btn-success',
            },
          })
          ...
          this.$store.dispatch('company/active', Obj).then(() => {})
        }
      })
    },
    confirmDelete(id) {
      this.$swal({
        ...
        if (result.value) {
          this.$swal({
            icon: 'success',
            title: 'عملية حذف',
            text: 'تم تفعيل هذا القاعة بنجاح!',
            customClass: {
              confirmButton: 'btn btn-success',
            },
          })
          this.$store.dispatch('company/delete', id).then(() => {})
        }
      })
    },
    showDetails(obj) {
